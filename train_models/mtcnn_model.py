...
def P_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        #batch*H*W*10
        landmark_pred = slim.conv2d(net,num_outputs=74,kernel_size=[1,1],stride=1,scope='conv4_3',activation_fn=None)
...
def R_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    with slim.arg_scope([slim.conv2d],
        ...
        print bbox_pred.get_shape()
        #batch*10
        landmark_pred = slim.fully_connected(fc1,num_outputs=74,scope="landmark_fc",activation_fn=None)
        print landmark_pred.get_shape()
        #train
        if training:
            cls_loss = cls_ohem(cls_prob,label)
