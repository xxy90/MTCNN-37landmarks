...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    if net == 'PNet':
        image_size = 12
        radio_cls_loss = 0.5;radio_bbox_loss = 0.5;radio_landmark_loss = 1.0;
    elif net == 'RNet':
        image_size = 24
        radio_cls_loss = 0.5;radio_bbox_loss = 0.5;radio_landmark_loss = 1.0;
    else:
    ...
    landmark_target = tf.placeholder(tf.float32,shape=[config.BATCH_SIZE,74],name='landmark_target')
    #class,regression
    ...
    try:
        for step in range(MAX_STEP):
            ...
            if i * config.BATCH_SIZE > num*2:
                ...
                i = 0
                t=epoch*2
                saver.save(sess, prefix+'%d.ckpt'%t)
                tf.train.write_graph(sess.graph_def, '../data/MTCNN_model/%s_landmark/t'%net,net+'.pb', as_text=False)
