...
class MtcnnDetector(object):
    ...
    def detect_onet(self, im, dets):
        ...
        #height
        h = boxes[:,3] - boxes[:,1] + 1
        landmark[:,0::2] = (np.tile(w,(37,1)) * landmark[:,0::2].T + np.tile(boxes[:,0],(37,1)) - 1).T
        landmark[:,1::2] = (np.tile(h,(37,1)) * landmark[:,1::2].T + np.tile(boxes[:,1],(37,1)) - 1).T        
    ...
    def detect_face(self, test_data):
        ...
        for databatch in test_data:
            ...
            im = databatch
            print ('the shape of loader image is ',im.shape)
            # pnet
            t1 = 0
            if self.pnet_detector:
                t = time.time()
