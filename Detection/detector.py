    ...
    def __init__(self, net_factory, data_size, batch_size, model_path):
        ...
        with graph.as_default():
            ...
            for key in var_to_shape_map:
                str_2=key[:-2]
                if not net.__contains__(str_2):
                    net[str_2]=reader.get_tensor(key)
                else:
                    net[str_2].append(reader.get_tensor(key))
            a=model_path.split('/')[-1].split('-')[0]
            np.save('/data/ai/xxy/MTCNN-Tensorflow-master/test/'+a+'.npy',net)                        
